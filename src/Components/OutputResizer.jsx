...
export default function Output_resizer({
  ...
  const isValidRecordNumber = useCallback((recordNumber) => {
    ...
    // Check if the record number contains only zeros

    if (/^0+$/.test(String(recordNumber))) {
  ...
  // Validate start record number and set error message

  const validateStartRecordNumber = useCallback((value) => {

    if (!value) {

      setStartRecordError('Startrecordnummer mag niet leeg zijn');
    ...
    if (!/^\d+$/.test(value)) {
      ...
      setStartRecordError('Alleen cijfers zijn toegestaan');
    ...
    if (/^0+$/.test(value)) {
      ...
      setStartRecordError('Startrecordnummer mag niet alleen uit nullen bestaan (0, 00, 000, etc.)');
    ...
    if (num > maxObjectValue) {

      setStartRecordError(`Startrecordnummer mag niet hoger zijn dan ${maxObjectValue}`);
  ...
  // We want: true when cStartNumber has valid digits (not empty, not only zeros)

  const validCStart = cStartNumber && /^\d+$/.test(String(cStartNumber)) && !/^0+$/.test(String(cStartNumber));
            ...
            {beeldbank && !isUploading && (
              ...
              <CheckDubbelRecords 
                ...
                onClearStartRecord={() => {
                  ...
                  // Clear the startrecordnummer when duplicates are found

                  setCStartNumber('');

                  setLocalCStartNumber('');

                  setStartRecordError('');

                  console.log('Startrecordnummer geleegd na dubbele records');
                    ...
                    <div className="mt-4">
                      ...
                      <input
                        ...
                        onBlur={() => {
                          ...
                          // Validate the input before updating parent state

                          const isValid = validateStartRecordNumber(localCStartNumber);

                          

                          if (setCStartNumber && isValid) {

                            // Only update if validation passes

                            setCStartNumber(localCStartNumber, selectedOverrideType);
                            ...
                            console.log('cStartNumber NIET bijgewerkt - validatie mislukt');
                            ...
                            if (selectedOverrideType && (selectedOverrideType === 'C' || selectedOverrideType === 'E')) {
                              ...
                              setCStartNumber(cStartNumber, selectedOverrideType);
                        ...
                        onChange={(e) => {
                          ...
                          // Validate the input and update error message
                        ...
                        className={`w-full border rounded ${startRecordError ? 'border-red-500 focus:border-red-500 focus:ring-red-500' : 'border-gray-300 focus:border-blue-500 focus:ring-blue-500'}`}
          ...
          <div key={statusBarKey} className="mb-4 bg-blue-200 p-2 rounded border border-gray-200 shadow-sm" style={{ height: '50px' }}>
            ...
            <div className="flex justify-between items-center h-full text-xs">
              ...
              <span className="text-gray-700">
                ...
                <span className="font-medium">Upload 100pc :</span>

                <span className="ml-1">{!appConfig.disable100pcUpload ? 'aan' : 'uit'}</span>
